...
func MountLogic(target string) {
	...
	os.WriteFile(filepath.Join(ConfigDir, "UDC"), []byte(""), 0644)
	...
	os.WriteFile(filepath.Join(ConfigDir, "idVendor"), []byte(CurrentVID), 0644)
	os.WriteFile(filepath.Join(ConfigDir, "idProduct"), []byte(CurrentPID), 0644)
	os.WriteFile(filepath.Join(ConfigDir, "bcdUSB"), []byte("0x0320"), 0644) // USB 3.1
	os.WriteFile(filepath.Join(ConfigDir, "bDeviceClass"), []byte("0x00"), 0644)
	os.WriteFile(filepath.Join(ConfigDir, "bDeviceSubClass"), []byte("0x00"), 0644)
	os.WriteFile(filepath.Join(ConfigDir, "bDeviceProtocol"), []byte("0x00"), 0644)
	os.WriteFile(filepath.Join(ConfigDir, "bcdDevice"), []byte("0x0100"), 0644)
	os.WriteFile(filepath.Join(ConfigDir, "bMaxPacketSize0"), []byte("0x40"), 0644)
	fmt.Printf("%s[OK]%s\n", ui.B_GREEN, ui.RESET)

	fmt.Printf("  %s├─ 覆写描述符 [%s%s%s]... %s", ui.G_TEXT, ui.B_CYAN, CurrentMan, ui.G_TEXT, ui.RESET)
	os.WriteFile(filepath.Join(ConfigDir, "strings/0x409/manufacturer"), []byte(CurrentMan), 0644)
	os.WriteFile(filepath.Join(ConfigDir, "strings/0x409/product"), []byte(CurrentProd), 0644)
	os.WriteFile(filepath.Join(ConfigDir, "strings/0x409/serialnumber"), []byte(CurrentSer), 0644)
	fmt.Printf("%s[OK]%s\n", ui.B_GREEN, ui.RESET)
	...
	os.WriteFile(filepath.Join(funcPath, "lun.0/nofua"), []byte("1"), 0644)
	os.WriteFile(filepath.Join(funcPath, "lun.0/removable"), []byte("1"), 0644)
	os.WriteFile(filepath.Join(funcPath, "stall"), []byte("1"), 0644)
	if _, err := os.Stat(filepath.Join(funcPath, "num_buffers")); err == nil {
		os.WriteFile(filepath.Join(funcPath, "num_buffers"), []byte("4"), 0644)
	}
	...
	if _, err := os.Stat("/dev/loop-control"); err == nil {
		...
		if out, err := cmd.Output(); err == nil {
			...
			if dev != "" {
				...
				os.WriteFile(fmt.Sprintf("/sys/block/%s/queue/logical_block_size", loopNum), []byte("4096"), 0644)
				os.WriteFile(fmt.Sprintf("/sys/block/%s/queue/nr_requests", loopNum), []byte("128"), 0644)
				os.WriteFile(fmt.Sprintf("/sys/block/%s/queue/scheduler", loopNum), []byte("noop"), 0644)
				os.WriteFile(fmt.Sprintf("/sys/block/%s/queue/read_ahead_kb", loopNum), []byte("256"), 0644)
				fmt.Printf("%s[DONE]%s\n", ui.B_GREEN, ui.RESET)
			}
		} else {
	...
	// 绑定文件
	if useLoop {
		if err := os.WriteFile(filepath.Join(funcPath, "lun.0/file"), []byte(loopDevice), 0644); err != nil {
	...
	if !useLoop {
		os.WriteFile(filepath.Join(funcPath, "lun.0/file"), []byte(target), 0644)
	}
	...
	vendorBytes := fmt.Sprintf("% -8s", vendor) // Pad with spaces
	prodBytes := fmt.Sprintf("% -16s", prod)
	
os.WriteFile(filepath.Join(funcPath, "lun.0/vendor"), []byte(vendorBytes), 0644)
	os.WriteFile(filepath.Join(funcPath, "lun.0/model"), []byte(prodBytes), 0644)
	os.WriteFile(filepath.Join(funcPath, "lun.0/rev"), []byte("1.00"), 0644)
	...
	if err := os.WriteFile(filepath.Join(ConfigDir, "UDC"), []byte(UdcName), 0644); err == nil {
		time.Sleep(2 * time.Second)
		speed := GetUSBSpeed()
		if speed != "UNKNOWN" {
